...
ggplot(mib_ypc_by_yardline, aes(x = DefendersInBox, y = yard_bin, fill = aly)) +
   geom_tile() +
   scale_fill_distiller(palette="Greens",
                        direction = 1) +
   geom_tile(color = "black") +
...
model_aly <- lm(data = mib_ypc_by_yardline, aly ~ yard_bin + DefendersInBox)
summary(model_aly)
...
ggplot(data = mib_ypc_by_yardline, aes(x = predict, y = ypc)) +
   ...
   geom_point(alpha = .4,
              size = 4) +
   theme_538 +
   labs(x = "Predicted Yards per Carry Using Just Defenders in the Box and Field Position",
...
team_yards_over_expected <- mib_grouped %>%
   ...
   mutate(yards_over_expected = yards - predict,
          ...
          scramble = ifelse(str_detect(playDesc, "scramble") == 1, 1, 0)) %>%
   filter(playType == "RUSH") %>%
   select(yard_bin, teamName, yards, predict, epa.x, yards_over_expected, aly_mib, aly_over_expected, season) %>%
   na.omit() %>%
   group_by(teamName, season) %>%
   summarise(ypc = round(mean(yards), 1),
...
team_counts_by_mib <- mib_grouped %>%
   ...
   filter(playType == "RUSH",
          scramble == 0) %>%
   select(DefendersInBox, teamName, yards, epa.x, yards_over_expected, season) %>%
   na.omit() %>%
   group_by(DefendersInBox, teamName, season) %>%
   summarise(rushing_yards = sum(yards),
...
ggplot(six_defenders_and_eleven, aes(x = season, y = count, group = Metric, color = Metric)) +
   ...
   labs(x = "Season", y = "Number of Rushing Plays (excluding QB scrambles)",
...
teams <- mib_grouped %>%
   ...
   mutate(yards_over_expected = yards - predict,
          scramble = ifelse(str_detect(playDesc, "scramble") == 1, 1, 0),
          aly_mib = ifelse(yards < 0, yards * 1.2, ifelse(yards %in% seq(0:4), yards, ifelse(yards %in% seq(5:10), yards * 0.5, 0)))) %>%
   filter(playType == "RUSH",
          scramble == 0) %>%
   select(DefendersInBox, teamName, yards, epa.x, aly_mib, yards_over_expected, season) %>%
   na.omit() %>%
   group_by(teamName, season) %>%
   summarise(rushing_yards = sum(yards),
             ypc = round(mean(yards), 1),
